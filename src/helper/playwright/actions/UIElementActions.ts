...
export default class UIElementActions {
  ...
  public async isEditable(sec: number): Promise<boolean> {
    ...
    return await element.isEditable({
      timeout: sec * CommonConstants.ONE_THOUSAND,
    });
  }
  ...
  public async isVisible(sec: number): Promise<boolean> {
    ...
    try {
      visibility = await this.getLocator().isVisible({
        timeout: sec * CommonConstants.ONE_THOUSAND,
      });
    } catch (error) {
      visibility = false;
  ...
  //     Log.info(`Clicking on ${this.description}`);
  //     await this.waitTillVisible();
  //     await this.getLocator().evaluate((node: HTMLElement) => {
  //       node.click();
  //     });
  //     return this;
  //   }
