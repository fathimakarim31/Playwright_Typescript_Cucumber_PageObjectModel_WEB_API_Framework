...
When(
  "user makes a request to retrieves all the Authors in the System",
  async function () {
...
When(
  "user makes a request to retrieve an Author with id {int}",
  async function (id: number) {
    const endPoint = `${process.env.REST_API_BASE_URL}${StringUtil.formatString(
      Constants.SINGLE_AUTHOR_EP,
      id.toString(),
      this.id
    )}`;
    this.response = await this.rest.get(
